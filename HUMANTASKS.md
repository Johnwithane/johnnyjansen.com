...
- [ ] **First deploy** from a machine logged into Firebase: `npm run deploy:rules && npm run deploy:functions && npm run deploy:hosting`. Open `https://familyhub-prod.web.app` (the portfolio) and `/app` (the portal), sign in with the owner account, tap Refresh on Today.
