...
Phase 2 progress (2026-09-20): **2a shipped**: the household calendar (`events`, wall-clock strings, kinds, members, rules with tests), the suggestions queue (`suggestions`, function-written, member-resolved, private ones visible to their owner only) with the Review screen that applies through the normal services, a Calendar screen (week strip; household events all week, Google for today), Plan tabs, the day's household events on Today and in the digest's Family section, and `me events / event / review / dismiss`. **2b shipped**: Vertex AI Gemini in functions (`lib/vertex.ts`, JSON salvage for truncated output, per-household daily cap in function-only `rateLimits/`), `analyzeReceipt` (photo or PDF in, a transaction proposal out, writes nothing), Money in the portal (accounts shared or private, transactions with categories mapped to CRA lines, monthly envelopes, receipt capture that shrinks the photo on the phone and files it under the person's own receipts folder), and the second factor enforced end to end: rules, Storage, the callable and the router all read the same `sign_in_second_factor`, a child never sees the Money tab. **2c shipped**: intake (approved senders per adult at More → Intake, a 06:00 daily scan and Scan now over new mail from those senders only, Gemini proposes events / tasks / bills / contacts / documents in the app's own vocabulary, everything lands on Review, bodies never stored, one read per message), bills and subscriptions (`bills`, sorted by yearly cost, due this week on Money and in the digest's Money section, the daily roll of past due days, creep flags from recorded transactions: amount went up, or no charge in 90 days), the load ledger's first field (`responsibleUid` on every bill, with the split shown on the Bills screen), bank CSV import (headered or headerless exports, category guess per line, already-recorded lines skipped), Review accepts bills and transactions, and `me bills / bill / scan / senders`. Minting a me token needs a second-factor session now. **2d shipped**: the wizard's "What we found" (`setupScanInbox`: 90 days of senders and subjects condensed to one line per pattern, one Gemini call, grouped into bills and subscriptions, accounts, upcoming bookings and clients; never bodies, never stored, two runs a day, re-running proposes only what is not already set up or waiting) and the Money starter (accounts by name, envelopes proposed from the bills, the rest left to size), with `planSetup` deciding the steps per person (the invited adult skips the household questions; no Google, no scan) and Review's appliers shared with the wizard so a tick is rule-gated exactly like a button. Phase 2 code is complete. Left for later: the forward address (needs the product domain), Businesses (Phase 3).
...
**Phase 2 (2026-09-20).** Two read-only adversarial reviews (rules + Storage, then functions + me). Fixed: the budget doc sat outside the second-factor gate; a scan timestamp could be erased by the client; approved-sender elements were untyped and an all-punctuation sender made the Gmail query match the whole mailbox; `source` (provenance) was client-writable on events and relabel-able on tasks; SVG passed the image filters; feedback fields reaching a GitHub issue were unbounded; the me endpoint skipped the rule on `feedback.triage` (adults), `events.delete` (owner or adult), `tasks.delete` (owner, or adult and shared) and `bills.add` (responsible adult must be a member); a me token followed its owner into another household; bills leaked into first-factor-readable snapshots and digests; raw Google and Vertex error objects (which carry the request, i.e. mail) were logged; the daily AI cap rolled at UTC midnight and the wizard scan was counted per person under the household's key; the intake scan never paged and could skip mail forever, and a timeout re-sent read mail to the model; `digest.run` had no cap; mail headers reached suggestion payloads unbounded; the email body reached the model unfenced. Decided and kept: intake proposals are household-visible (approving a sender is the consent; the Intake screen says so); receipts are readable by their uploader only in Storage (a shared receipt another adult needs will go through a function); the me token is the credential once minted under a second factor and does not expire yet. Backlog from the review: an adult-only "revoke someone else's token" plus `lastUsedAt` and a token age limit; a lease between the scheduled and the manual intake scan; a `businesses` rule before Phase 3 writes one.
