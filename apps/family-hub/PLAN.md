...
Phase 2 progress (2026-09-20): **2a shipped**: the household calendar (`events`, wall-clock strings, kinds, members, rules with tests), the suggestions queue (`suggestions`, function-written, member-resolved, private ones visible to their owner only) with the Review screen that applies through the normal services, a Calendar screen (week strip; household events all week, Google for today), Plan tabs, the day's household events on Today and in the digest's Family section, and `me events / event / review / dismiss`. Next: 2b receipts + Money, 2c intake + bills.
