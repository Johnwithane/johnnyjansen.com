...
- Intake reads mail from senders the person approved, once, and stores only what it proposed plus the message id (`users/{uid}/private/intakeSeen`). Never store a body, never widen the scan past `intake.senders`.
