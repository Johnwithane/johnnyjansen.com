...
SceneGame = game.Scene.extend({
    ...
    showScore: function() {
        var box = new game.Sprite('media/gameover.png', game.system.width / 2, game.system.height / 2, {anchor: {x:0.5, y:.88}});
        ...
        highScoreText.position.x = 67;
        highScoreText.position.y = -100;
        box.addChild(highScoreText);

        var scoreText = new game.BitmapText('0', {font: 'Pixel'});
        scoreText.position.x = highScoreText.position.x = 40;
        scoreText.position.y = -157;
        box.addChild(scoreText);

        game.scene.stage.addChild(box);

        this.restartButton = new game.Sprite('media/restart.png', game.system.width / 2, game.system.height / 2 + -10, {
            anchor: {x:0.5, y:0.3},
            scale: {x:0, y:0},
            interactive: true,
            mousedown: function() {
            ...
            this.addTimer(time, function() {
                ...
                if(scoreCounter >= game.scene.score) {
                    ...
                    if(game.scene.score > highScore) {
                        ...
                        var newBox = new game.Sprite('media/new.png', -138, -90);
                        box.addChild(newBox);
                    }
